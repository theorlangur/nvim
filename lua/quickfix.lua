function QuickFixMappings(args)
    ...
    vim.keymap.set('v', '<Space>f', 'y:Cfilter <C-r>0<CR>', {desc="Filter in quickfix with current visual selection", buffer= args.buf})
    vim.keymap.set('v', '<Space>F', 'y:Cfilter! <C-r>0<CR>', {desc="Filter out quickfix with current visual selection", buffer= args.buf})
end
